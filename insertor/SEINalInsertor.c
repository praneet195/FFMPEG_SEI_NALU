 ...
 */
volatile int16_t marker=0;
#define RBCCPS_ARR_SZ 151
...
char tempstr[300];
char complete[300];
sprintf(tempstr,"Frame Marker: %d\n",marker);
strcat(complete,tempstr);
unsigned char strH[600];
int ii,jj;
tempstr[0]="\0";
for(ii=0,jj=0;ii<strlen(complete);ii++,jj+=2)
    { 
        sprintf((char*)strH+jj,"%02X",complete[ii]);
    }
strH[jj]='\0'; /*adding NULL in the end*/
uint8_t imn=0;     
int num;
uint8_t* sei=(uint8_t*)malloc(sizeof(uint8_t)*RBCCPS_ARR_SZ);    
for(uint8_t seipkt=0;seipkt<RBCCPS_ARR_SZ;seipkt++){
if(seipkt<3){
sei[seipkt]=(uint8_t)0;
}
else if(seipkt==3){
sei[seipkt]=(uint8_t)1;
}
else if(seipkt==4){
sei[seipkt]=(uint8_t)6;
}
else if(seipkt==5){
sei[seipkt]=(uint8_t)5;
}
else if(seipkt==6){
sei[seipkt]=(uint8_t)128;
}
else{
sei[seipkt]=255;
if(imn<strlen(strH)-2){
char *temp=strndup(strH+imn,2);
sscanf(temp,"%x",&num);
sei[seipkt]=(uint8_t)num;
//printf("%d ",num);
imn=imn+2;
}
}
...
for (i = 0; i < pkt->size; i++) {
        newbuf[k]=pkt->data[i];
        if(pkt->data[i+1]==0&&pkt->data[i+2]==0&&pkt->data[i+3]==1&&((pkt->data[i+4]&xorbit)==0x05)){
		marker++;
